...
A tab-delimited `.tsv` file containing coordinates can be passed into `roi_file`. The file must contain three columns, and the first row should contain the column names, which must be: 'x', 'y', and 'z'. For instance:
...
It is often desirable to only extract signal from gray matter voxels within the sphere instead of all voxels (which can include white matter and non-brain voxels). Here, you can pass in a mask (e.g., a subject's gray matter mask) to `mask_img` (`-m` or `--mask_img`), which will take the intersection of this mask and the spheres generated by `niimasker`. Therefore, each region you are extracting from will contain only gray matter. If your mask is specific to each subject, then you will need to run niimasker separately for each subject (see *Working with single ROI masks* for an example python script that iteratively runs niimasker).

The spheres, or sphere intersects if `mask_img` is used, are saved to `niimasker_data/spheres_img.nii.gz`.
